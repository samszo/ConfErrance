...
var son, pSon, slides={}, svg,
    slide = 0,
    delay = delay ? delay : 5000,
	fct={};

function svg_slides(s,delay,outFct) {
	fct=outFct;
	svg = s;
	//style du body
	d3.select("body")
    ...
    svg.selectAll("rect").each(function(d,i){
        ...
        if (id.slice(0,6)=='slide_') { 
            console.log(id);
            slides[id.slice(6)]=this;
            //rects[i].scale_x = d3.scaleLinear().range([this.x.baseVal.value,this.x.baseVal.value+this.width.baseVal.value]).domain([0,1000]);
            //rects[i].scale_y = d3.scaleLinear().range([this.y.baseVal.value,this.y.baseVal.value+this.height.baseVal.value]).domain([0,1000]);
        }
...
function next_slide()  {
	...
	/*ATTENTION il ne faut pas un id invalide par exemple slide_1.2 au lieu de slide_1_2
	let rct = d3.select('#'+slides[keys[slide]].id);
	if(rct.attr('iframe')){
		showWebPage(rct.attr('iframe'));
	}
	*/
